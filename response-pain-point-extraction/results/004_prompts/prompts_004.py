"""
Shared prompts for the Pain Points POC - Version 4.

Changes from V3:
- Source references: Document name ONLY (no page numbers - can't verify accuracy)
- Option 2 Step 1: UP TO 15 pain points (was 10) - test if more options improves filtering
- Threshold will be 0.85 (compromise between V2's 0.9 and V3's 0.8)
- Keep verification step for Option 3

V3 prompts preserved in: prompts_003.py
...
4. source_references (use EXACT document names from list above, format: ["DocumentName.pdf"])
...
Extract UP TO 15 potential buyer pain points. We'll refine in the next steps.

Cast a wide net to capture all potential strategic challenges. Quality filtering happens in Step 2.

For each pain point, provide:
1. title (max 50 chars)
2. description (max 300 chars - include WHAT the challenge is, WHY it matters, CONTEXT if mentioned)
3. source_references (use EXACT document names from list above, format: ["DocumentName.pdf"])
